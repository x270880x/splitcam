...
**Счётчик VirusTotal 12.08: 4/91 утром → 2/91 днём → Fortinet снят вечером.**
За день отвалились AlphaSOC, CRDF и Fortinet. Данные свежие, не кэш: переанализ на VT показывал
«25 minutes ago».

| Вендор | Статус | Что сделано | Проверка результата |
|---|---|---|---|
| **Fortinet** | ✅ **СНЯТ 12.08** — переклассифицирован в `Freeware and Software Downloads` | заявка подана 12.08 в 04:20 PDT, ответ пришёл в 05:28 PDT — час с небольшим | уведомление на `support@`; на устройствах может держаться кэш веб-фильтра |
...
**Владение доменом подтверждено 12.08.2026.** Портал потребовал файл (не TXT-запись),
положен в докрут на `77.83.100.153`:

    gridinsoft-qnz7ipetj2jbi7u010d1uvokoxpw0mnwuch3pwvtpgqiippviyakl8tu4s4sff0d.txt

Внутри — сам токен, 64 байта. Отдаётся с HTTP 200 и `text/plain` и через Cloudflare,
и напрямую с origin. **Файл не удалять**, пока тикет открыт: порталы обычно
перепроверяют владение. Временная TXT-запись, добавленная до того, как выяснился
нужный метод, удалена.
