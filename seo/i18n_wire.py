...
PAGE_PATHS = ["", "products/", "features/", "download/", "virtual-camera/", "virtual-audio-mac/", "multistreaming/", "alternatives/",
...
PRIO = {"": "1.0", "multistreaming/": "0.9", "virtual-camera/": "0.9", "virtual-audio-mac/": "0.8", "products/": "0.9", "features/": "0.9", "download/": "0.9",
...
def wire_file(locale, page, avail):
    ...
    # 4b. RTL layout fixes for RTL locales (arrow flip + split-button radii), kept in a
    # regenerated marker region so edits to i18n.RTL_CSS propagate on the next wire run.
    if i18n.is_rtl(locale):
        block = region("RTLCSS", "\n" + i18n.RTL_CSS + "\n")
        if re.search(r"<!--RTLCSS-->.*?<!--/RTLCSS-->", s, re.DOTALL):
            s = re.sub(r"<!--RTLCSS-->.*?<!--/RTLCSS-->", lambda _: block, s, flags=re.DOTALL)
        else:
            s = s.replace("</style>", "\n" + block + "\n</style>", 1)
