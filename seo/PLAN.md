...
## 🆕 DroidCam и /for/gamers/ — обе отклонены, 2026-09-06

Обе прогнаны через ту же связку «аналитик + скептик». **Аналитик и скептик сошлись: reject у обеих**,
и оба развода запросов признаны неработающими (`split_holds: false`).

### DroidCam — отклонена: одно торговое предложение не тянет две страницы

Спрос 900/мес по всем базам, KD 21 — это больше, чем у построенных restream (390) и streamyard (320).
Но форма спроса и досягаемость его убивают.

- **Единственное честное отличие уже потрачено.** «Ничего не ставить на телефон» — это дословно title
  и H1 существующей `/phone-as-webcam/`. Две страницы нельзя развести токеном в заголовке, если их
  торговое предложение — одно и то же предложение.
- **Страница противоречила бы живой соседке.** `/phone-as-webcam/` в 35 локалях прямо говорит: «если
  нужен просто телефон как обычная веб-камера, специализированный инструмент вроде DroidCam, Iriun или
  Camo — более прямой ответ, им и пользуйтесь». Страница-конкурент обязана спорить с этим абзацем.
- **Таблица сравнения уже есть** там же — «SplitCam vs a single-purpose webcam app», восемь строк, и это
  ровно сравнение с DroidCam. Дубль на 35 локалей запрещён правилом PAGE-SIMILARITY.
- **Измерено, а не оценено:** за 90 дней весь кластер droidcam дал домену 14 показов, 0 кликов, средняя
  позиция 24,1, и все — на главной. Для калибровки в том же окне: restream 1017 показов, manycam 944,
  streamyard 688, nvidia broadcast 29, snap camera 22, vmix 16, droidcam 14, stream deck 0. **ManyCam —
  решающее сравнение:** Semrush оценил его в 50/мес, но «manycam alternative» приносил 346 показов на
  позиции 4,9 ещё до появления страницы. DroidCam в 18 раз крупнее по объёму и даёт в 20 раз меньше
  видимости, на третьей странице вместо первой. Это не нехватка контента, это Google относит нас к
  другой категории продуктов на этой выдаче — что `/phone-as-webcam/` и признаёт словами.
- **Ноль строк у `/phone-as-webcam/` в выгрузке объяснён:** страница создана 2026-09-04, окно выгрузки
  кончается 09-02. Её тогда не существовало. Живой запрос за 09-03…09-05 тоже пуст — ей два дня.

### /for/gamers/ — отклонена: спрос есть, но он уже чужой

Прежний вердикт «делать» был условным и требовал перемера по локалям в консоли. Перемер сделан и
переворачивает вывод.

- **Игровое намерение до домена не доходит.** Строгие запросы про захват игры: 53 показа за 90 дней,
  из них 44 — один артефакт LLM-персоны. Живых человеческих запросов **9 показов за 90 дней на весь
  мир**, то есть около 3 в месяц.
- **Twitch-намерение настоящее (около 476 показов в месяц), но занято целиком.** Разбор по странице:
  523 показа у `/es/ayuda/start/como-transmitir-en-twitch`, 157 у главной («twitch virtual cam», поз. 8
  — это намерение виртуальной камеры), 60 у русской help-страницы, остальное у `/multistreaming/` в
  fr/de/pt/it/ar. Ни один показ не принадлежит запросу, который страница для геймеров могла бы забрать,
  не отняв его у страницы, которая его уже имеет.
- **Честность страницу не спасает.** Все четыре опоры подтверждены в репозитории построчно: Game Capture
  для DirectX и OpenGL, аппаратное кодирование, источник Replay на горячей клавише, встроенный Tip
  Notifier. Проблема не в правде, а в новизне: всё это уже опубликовано на главной и отзеркалено в 35
  локалей, причём три из четырёх — внутри одного игрового блока.

### Состояние квот на 2026-09-06

Semrush отдаёт «does not have enough API units» на всех инструментах; Ahrefs израсходован — 99 996 из
100 000, сброс 2026-09-22. Объёмы в разборе выше взяты из кэша `seo/data/keywords-research.json`
(2026-05-20), и это честно помечено. Консоль работает и была основным источником.

### Итог: к постройке остаются ДВЕ страницы

1. **Snap Camera** — собрана и локализована на 35 языков 2026-09-06, лежит закрытой.
