...
> ⚠️ **С 2026-09-18 origin сайта — `88.218.29.75` (`pl-rocket-da4`), а не `77.83.100.153`.**
> Уже отправленные заявки ниже не трогать — это их дословный текст. В НОВЫХ заявках и
> проверках указывать новый адрес. Подробности: `seo/MIGRATION-2026-09-18.md`.
...
| AbuseIPDB по `77.83.100.153`: **Confidence of Abuse 3%**, всего 1 репорт | их страница, 12.08.2026. Единственный репорт от 29.07 от `xmission.com`, категория Port Scan, по тексту — срабатывание UFW на пакет с source port 443. ISP HOSTPRO LAB LLC, AS24723, Польша. **IP чист — причиной блокировок быть не может**, что подтверждает доменный характер листинга. Скриншот этой страницы приложить к заявке Malwarebytes |
| ThreatFox IOC 1861022 — полная карточка | их сайт, 12.08.2026, после прохождения hCaptcha пользователем. См. блок 0.2 |

**Что проверить не смог:** URLhaus (требует auth-key), поиск по форуму Malwarebytes
(требует входа), настройки Cloudflare (нет токена на этой машине).
...
### 0.1. ~~Проверить сайт через Cloudflare~~ — ✅ СДЕЛАНО 12.08.2026

Проверено по API Cloudflare (токен выдан владельцем, зона `splitcam.com`,
id `d78682c510a9fa3285c47bb52a11e0f4`). **Механизма выборочной подмены на уровне CDN
нет.** Разбор по слоям:

**Workers — 4 роута, один скрипт `cam-redirect`:**
`/help/*`, `/ru/help-ru/*`, `/es/ayuda/*`, `/blog/2020/*`. Исходник прочитан целиком
(8454 байта): 75 строк карты редиректов на `camstreamguide.com` плюс три строки логики —
нашли путь в карте, отдали `Response.redirect(target, 301)`, не нашли — `fetch(request)`
без изменений. Сгенерирован 10.06.2026 при переносе adult-раздела. Ни `eval`, ни
`new Function`, ни `atob`, ни `HTMLRewriter` (без него в HTML ничего не вставить), ни
единой проверки User-Agent или Referer — то есть клоакинг там невозможен технически.

**Transform rules — 3, все безобидные:** переписывание `.cfg` → `_cfg.bin` для
`proxy.cfg`, `ingests.cfg`, `ingests2.cfg`. HTML не трогают.

**Dynamic redirect — 1:** `www` → apex, 301. **Firewall custom — 2:** блокировки
легаси-мусора (`_splitcam5.php`, `adver`, `/insideads`). **Page Rules — 3:** только
настройки кэша.

**DNS:** проксируются ровно два имени — `splitcam.com` и `www`, оба на `77.83.100.153`.
`mail` и `webmail` не проксируются (так и надо). **Забытых поддоменов, указывающих на
старые серверы, нет** — это отдельно проверялось, потому что такой поддомен объяснил бы
репорт.

**Zaraz** — токену не хватило прав (`Authentication error`), но проверяется наблюдением:
если бы Zaraz что-то внедрял, его скрипт был бы в HTML. Живая главная 12.08.2026 грузит
единственный внешний скрипт-хост `www.googletagmanager.com`, `zaraz` в разметке не
встречается.

**Побочная находка — три transform-правила больше не нужны.** Их описание («новый хост
отдаёт 403 на .cfg») устарело: origin отдаёт `proxy.cfg` и `ingests.cfg` напрямую с
HTTP 200, проверено `curl --resolve` мимо Cloudflare. Это ровно тот TODO, что висит в
`ONBOARDING.md` с 06.07 («delete the 3 CF .cfg transform rules after 48h»). Удалять —
решение владельца, само по себе к репутации отношения не имеет.

**Что это значит для заявок:** версий, по которым сайт мог отдавать ClearFake выборочно,
не осталось ни одной из проверяемых. Продувку кэша Cloudflare (`Purge Everything`) всё
равно стоит сделать перед подачей — это единственное, что не читается через API.

### 0.2. Закрыть вопрос по ThreatFox IOC 1861022 — самый высокий рычаг

> ✅ **Карточка открыта и прочитана 12.08.2026** (пользователь прошёл hCaptcha).
> Реквизиты подтверждены полностью:
>
>     IOC ID:      1861022
>     IOC:         splitcam.com (тип domain)
>     Threat type: payload_delivery
>     Malware:     ClearFake      Теги: ClearFake, ClickFix
>     Confidence:  умеренная, 50%
>     Is compromised?: True
>     ASN:         AS13335 CLOUDFLARENET
>     First seen:  2026-07-27 19:57:13 UTC
>     Last seen:   НИКОГДА
>     Reporter:    skocherhan       Reference: отсутствует
>
> **Ключевой вывод: запись сделана 27 июля, то есть уже ПОСЛЕ перестройки сайта.**
> Снимок Wayback от 18.07.2026 весит 144 122 байта — ровно как нынешняя страница,
> без признаков инъекции, единственный внешний скрипт-хост `googletagmanager.com`.
> Снимок от 02.08 тоже чист. Снимка ровно на 27.07 в архиве нет (разрыв 18.07 → 02.08).
>
> Значит формулировка «остаточная репутация от старого взлома» неверна: на дату
> репорта сайт уже девять дней как был статикой. Правильная формулировка для заявки:
> **репорт одиночный, ни разу не переподтверждён (`last seen: never`), без ссылки на
> доказательства, с умеренной уверенностью 50%** — и сайт на ту дату уже был
> перестроен.
>
> ⚠️ Честная оговорка, которую надо держать в голове: ClearFake обычно отдаётся
> выборочно — по реферу, User-Agent и гео. Ни архив, ни наши запросы такого не
> поймают. Единственное непроверенное место, где выборочная подмена возможна на
> статике, — **уровень Cloudflare** (Workers, Zaraz/apps, протухший объект в кэше).
> Токена Cloudflare на этой машине нет, проверить нечем.
>
> Оспаривать здесь же: в карточке IOC есть действие **Actions → Report False
> Positive**, но оно требует аккаунта abuse.ch (сайт показывает «Login»).
> Регистрация — за владельцем.
