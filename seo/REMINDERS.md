...
| **2026-07-17** | ✅ 2026-07-17 SHIPPED — button live in 35 locales | SplitCam Remote **Android** — closed-testing tester CTA | `/products/` "Coming soon" chip → live "Become a tester" button (→ signup form), Step-02 prose updated, all 35 locales. Funnel: form → manual add to `web-testers` list → opt-in email. Full detail: `seo/ANDROID-TESTER-CTA.md` + `seo/TESTER-FUNNEL-COPY.md`. Play Console: feedback URL=splitcameramail@gmail.com (in review since 07-17), `web-testers` list created (attach staged). **Group approach dropped** (Testers radio is list-OR-group). Still TODO: engage the existing 12 testers (only 1 installed) — that, not headcount, is why Google rejected on 07-11. |
