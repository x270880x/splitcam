...
| — | 🔄 в работе | **Снять блокировки домена у 5 вендоров** — подано 12.08: Fortinet, MalwareURL, AlphaSOC. Осталось: CRDF (форма заполнена, нажать Send), ThreatFox (нужен аккаунт abuse.ch — это верхнее звено), Malwarebytes (аккаунт + скриншот). Статусы и сроки: — splitcam.com помечен как вредоносный: AlphaSOC, CRDF, Fortinet, MalwareURL (категория `Trojan JS ClearFake`), Gridinsoft. Живого заражения не найдено, флаг доменного уровня. `seo/APPEALS-READY.md`, разбор — `seo/REPUTATION-DELISTING.md`. Успех меряется счётчиком VirusTotal: было 4/91, цель 0 |
