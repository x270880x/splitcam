...
| Срок | Статус | Задача |
|---|---|---|
| **до отключения cPanel** | 🔴 риск потери данных | **Досинхронизировать почту за 6–14 июля.** ~8 дней писем на `admin@` и `pola@` осели на старом cPanel (MX ещё указывал туда) и на DirectAdmin НЕ перенесены — UAPI был недоступен при переезде. Отключение cPanel без этого = потеря. Подробности: [REMINDERS-LOG.md](REMINDERS-LOG.md), «MAIL CUTOVER to DirectAdmin» |
