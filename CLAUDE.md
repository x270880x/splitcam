...
3. **Комментарий с SEO-таргетингом** (2026-09-05) — те же 245 файлов несли
   ...
   что заточена страница. Исправлен пока только на 35 файлах vmix — остальные 210 открыты.

Почему каждый раз проскакивает: поле валидно синтаксически, ссылка рабочая, картинка
существует, комментарий вообще невидим. Ни одна проверка «страница не сломана» этого не видит —
видно только сравнением с ИСХОДНОЙ страницей шаблона.

**Правило:** нашёл одно поле, унаследованное от шаблона, — не чини его в одиночку. Продиффь весь
`<head>` (и комментарии в нём) новой страницы против страницы-шаблона и вычисти всё разом.
Волумы в таком комментарии не выдумывать: если для страницы спрос не мерили, так и писать.
