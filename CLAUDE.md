...
### linkcheck: вторая слепая зона — hreflang (найдена 2026-09-03)

Первая слепая зона была про абсолютные внутренние ссылки. Вторая нашлась при сборке
страниц `/alternatives/`: **hreflang не проверялся вообще**.

Страница, собранная из чужого шаблона, унаследовала 34 hreflang, указывающих на
локали страницы-шаблона. То есть `/alternatives/manycam/` объявлял своим французским
переводом страницу про OBS. Три страницы × 34 = 102 ложных объявления.

Почему это не ловилось: **ссылка полностью рабочая**. Файл на диске есть, код 200,
проверка ссылок довольна. Ошибка видна только по взаимности — настоящий перевод
обязан ссылаться обратно.

Правила:
- Новая страница без локалей объявляет **только** `en` + `x-default` на саму себя.
  Никогда не оставлять унаследованный от шаблона список локалей.
- `seo/linkcheck.py` теперь проверяет hreflang на существование цели И на взаимность.
  Оба типа поломки проверены внедрением дефекта, а не предположением.
- Корневой URL пишется как `https://splitcam.com/`, остальные без слеша — при
  сравнении нормализовать, иначе главная ложно помечается невзаимной.

Общий вывод, уже второй раз: **если инструмент говорит «0 проблем», сначала проверь,
что он вообще умеет видеть эту проблему.** Оба раза «0» означало «не искал».

### 🔴 Обратная половина того же правила: проверка, которая кричит о норме (2026-09-05)

За одну сессию три раза подряд МОЯ ЖЕ проверка сообщила о дефекте, которого не было.
Каждый раз это стоило времени и едва не привело к «починке» исправного:

1. **Подстрока вместо элемента.** После удаления `<p class="sec-p">` из разметки я искал
   на живом сайте подстроку `sec-p` — она осталась в CSS-правиле `.sec-p{…}`. Вывод:
   «не обновились все 35 локалей». Обновились.
2. **Класс, законный во втором месте.** На `/features/` надзаголовок `sec-eyebrow` есть
   и у блока FAQ внизу. Проверка «надзаголовков быть не должно» показала провал на всех
   девяти локалях. Правильная проверка — «было 2, стало 1».
3. **Переполнение без учёта прокручиваемого предка.** 38 «вылезающих» элементов на
   `/alternatives/obs/` оказались ячейками таблицы сравнения внутри
   `.table-wrap{overflow-x:auto}` — она шире экрана НАМЕРЕННО и скроллится сама.
   Страница вбок не едет: `scrollX` остаётся 0.

**Правило: прежде чем чинить найденное проверкой, убедись, что проверка отличает дефект
от нормы.** Проверять надо структуру (тег с классом), а не подстроку; считать разницу
(«было 2 → стало 1»), а не наличие; для геометрии учитывать предков.

Рабочая проверка переполнения (после неё 40 страниц, включая ar/he/fa, дали 0):
элемент — проблема, только если он выходит за `clientWidth` И ни один предок до
`<html>` не имеет `overflow-x` в `auto|scroll|hidden|clip`. Плюс контрольный опыт:
`window.scrollTo(9999,0)` и `scrollX` должен остаться 0.

### Возможности, подтверждённые владельцем продукта (2026-09-03)

Разбор по мультикамере проверял утверждения ТОЛЬКО по тексту сайта и поэтому пометил
часть функций как «нигде не заявлено». Владелец подтвердил, что они есть. Витрина
про них молчит — это дыра в контенте, а не отсутствие функции.
...
**Правило на будущее:** отсутствие функции в текстах сайта ≠ отсутствие функции в
продукте. Прежде чем писать «не поддерживается», спроси владельца. Обратное тоже
верно: писать можно только то, что подтверждено — сайтом, changelog’ом или владельцем.

### Поправка к правилу реальных запросов (2026-09-03)

Правило «сначала реальные запросы из Search Console» имеет границу применимости,
на которой я едва не ошибся.

Консоль показывает только запросы, по которым сайт **уже показывался**. Если контента
по теме нет вообще, показов не будет — и ноль в консоли не означает отсутствия спроса.

- `/for/gamers/` — 3 показа: данные ОСМЫСЛЕННЫ, сайт показывался, Google нас рассматривал.
- Stream Deck — 0 показов: данные ПУСТЫ, про Stream Deck на сайте нет ни строчки.
  Внешняя проверка (Ahrefs) показала 1240 запросов/мес при сложности 0–2.

**Порядок такой:** консоль отвечает на вопрос «доходит ли до нас существующий спрос».
На вопрос «есть ли спрос вообще» она не отвечает — для новой темы нужен внешний источник.

### Телефон как камера: механизм подтверждён владельцем (2026-09-03)

Сайт описывал это двумя противоречащими способами. Владелец подтвердил:

**Подключение по QR идёт через сервер. Через сервер идёт ТОЛЬКО установка соединения.**

То есть QR-код и связывание телефона с компьютером проходят через инфраструктуру
SplitCam, а сам видеопоток — нет. Это снимает противоречие между «Wireless · LAN» на
главной и «country specific servers for Remote Camera source» в changelog v10.8.70:
серверы обслуживают подключение, а не передачу видео.

**Как писать на страницах:** не утверждать «всё остаётся в вашей локальной сети» —
это неверно для этапа подключения. Формулировать честно: связывание идёт через наш
сервер, видео — нет.

### 🔴 Третий за день случай одной ошибки: жадная регулярка с re.S (2026-09-03)

За один день одна и та же ошибка сломала три разные вещи:

1. **Извлекатель строк** — `<span class="eyebrow">(.*?)</span>\s*<h4>` откатился от шапки
   и захватил полстраницы: в «строку» попало 8898 символов.
2. **Сборщик локалей** — тот же шаблон вставил hero внутрь карточки related.
3. **Активация карточек хаба** — `<div class="hub-card soon">…(.*?<h3>ИМЯ</h3>.*?)</div>`
   начала с ПЕРВОЙ карточки-заглушки и дотянулась до нужного `<h3>` через соседнюю.
   Результат: «Streamlabs» вёл на /restream, «Restream» на /streamyard,
   «StreamYard» на /manycam. **Уехало на живой сайт.**

Общий механизм: `(.*?)` ленив, но при неудаче движок ОТКАТЫВАЕТСЯ и расширяет захват.
С флагом `re.S` расширяться можно через любые теги и переводы строк — то есть через
всю страницу. Ошибка не выглядит ошибкой: групп столько, сколько ждали, HTML остаётся
валидным, ссылки рабочие.

**Почему не поймали проверки:** я сверял, что ссылки ведут на существующие файлы (вели),
и что живых карточек 5, а заглушек 2 (так и было). Сверить ЗАГОЛОВОК карточки с ЕЁ ЖЕ
ссылкой не догадался.

Правила:
- Никогда не искать по классу, который встречается больше одного раза на странице
  (`eyebrow`, `hub-card`), без якоря на уникальный родительский элемент.
- `[^<]*` вместо `(.*?)` там, где содержимое заведомо без тегов.
- `re.S` не ставить, если шаблон не должен пересекать строки.
- Для повторяющихся блоков резать по ГРАНИЦАМ (позиции начал), а не сопоставлять
  открывающий и закрывающий теги — вложенность ломает обратные ссылки `</\1>`.
- `seo/linkcheck.py` теперь сверяет заголовок каждой карточки хаба с её ссылкой
  (245 карточек по 35 хабам). Проверено внедрением дефекта.

### Длины title/description считать кодом, не на глаз (2026-09-04)

Три раза за день «подрезанное» описание уходило в коммит длиной 180, 165 и снова 165 при
пороге 160. Правило: кандидаты формулировок — список, длина считается в коде, берётся
первый в диапазоне 140–160; на страницу пишется только измеренная строка. Сборщик страниц
принимает текст как есть — он длину не проверяет, проверка лежит на том, кто пишет копию.

### 🔴 Агент повредил файл локали, а я принял это за «старый пробел» (2026-09-04)

`hr/for/churches/index.html` после прохода «правка утверждений о vMix в 34 локалях» лишился
`<table>`, `</tbody>` и нескольких закрывающих `</div>`: 7–8 строк разметки были перезаписаны
случайными строками того же файла (FAQ-абзацы, tip-карточки). Перевод остался цел.

Что пошло не так у меня: проверочный скрипт нашёл «0 строк таблицы» в hr, и я записал в
REMINDERS «в хорватской локали таблицу никогда не переносили». Это была неверная гипотеза,
принятая без проверки по git (`git log -- hr/for/churches/index.html` показал бы, что файл
менялся сегодня). Поломка ушла на живой сайт в 18:22 и провисела до вечерней выкатки.

Почему не ловилось: ссылки целы, JSON-LD парсится, FAQ совпадает с разметкой, счётчики
отдельных тегов близки к норме. Видно только по ПОРЯДКУ блочных тегов относительно EN.

Правила:
- Локальная страница без блока, который есть в EN, — сначала `git log` по файлу, потом
  гипотезы. «Не переносили» и «сломали сегодня» лечатся по-разному.
- `seo/linkcheck.py` сверяет порядок блочных тегов каждой локали с EN (680 страниц), порог
  ≥3 расхождений; инлайновые теги и changelog исключены. Проверено на реальной
  повреждённой версии из git (8 расхождений → поймано) и на починенной (0).
- Агент, правящий локализованный файл точечно, обязан после правки сверить структуру
  с EN сам — это добавлять в промпт каждого такого прохода.
- В zsh `$VAR:h` — модификатор пути, не «переменная и двоеточие». `git show "$C:path"` с
  переменной без фигурных скобок молча превращается в мусор, а `> файл` при этом успевает
  ОБНУЛИТЬ файл. Всегда `"${C}:path"` и никогда не перенаправлять в рабочий файл.

### 🔴 `git add -A` при работающих агентах — запрещено (2026-09-05)

Агент локали ko записал свой `ko/alternatives/stream-deck/index.html` в КОРЕНЬ репозитория —
поверх английской главной (144 КБ → 60 КБ, `<html lang="ko">`, canonical на /ko/…). Мой
`git add -A` через минуту закоммитил это как «wording: homepage doubled phrase removed».
Не выкатилось только потому, что деплоя между этими событиями не было.

Правила:
- Пока идёт workflow, коммитить только ЯВНЫЕ пути (`git add ru/... es/...`), никогда `-A`.
- После любого прохода агентов по локалям — скан корневых страниц: `<html lang` должен быть
  `en`, canonical без `/xx/`. Это уже делает блочная сверка в linkcheck (она и поймала:
  «34 главных расходятся» означало, что изменилась английская).
- В промпт агента, пишущего в файл: путь назначения повторить дважды и потребовать проверить
  `git status` — изменён ровно один ожидаемый файл.

### 🔴 zsh не разбивает `$LOCS` на слова — укусило дважды (2026-09-05)

`for l in $LOCS; do git mv "$l/…"` в zsh даёт ОДНУ итерацию со строкой «ru es de … fa» целиком
и падает. Второй раз это оборвало сессию посреди переноса /stream-deck/. Циклы по локалям —
только в python (`LOCS="…".split()`) или с `${=LOCS}`. Для деплоя и точечных правок в bash
списки локалей не использовать вовсе.

### 🔴 Каждый перевод проверяет носитель-филолог, который сам стримит (правило владельца, 2026-09-05)

Перевод на любой язык не готов, пока его не вычитал второй агент — носитель уровня профессора
филологии этого языка И практик стриминга. Не переводчик. Полное описание прохода — в скилле
`splitcam-localize`, раздел «Обязательная вычитка носителем». Правило действует на всё, что
уходит в локали: страницы, блоки, фразы, ссылки-подписи.

Долг, сверено по git 2026-09-06 (предыдущая формулировка была неточной — см. ниже).

**Текст страницы вычитан носителем:**
- `/alternatives/manycam/`, `/restream/`, `/streamyard/`, `/stream-deck/` — проход 2026-09-04
  (коммит `14413694`, 34 локали каждой).
- `/alternatives/vmix/` — проход 2026-09-06 (коммит `0c6293d1`): 235 правок в 34 локалях,
  чистой не оказалось ни одной. Кальки 81, грамматика 63, термины 38, пунктуация 19,
  ложные друзья 16. Подтвердилась закономерность прошлого прохода: в большинстве случаев
  правильное слово УЖЕ стояло на той же странице, переводчик придумывал второе.

- `/multi-camera/` — проход 2026-09-06 (коммит `0c7824b9`): **534 правки** в 34 локалях,
  чистой снова ни одной. Кальки 162, грамматика 134, термины 118, пунктуация 38, ложные
  друзья 37, факты 17. Вдвое больше, чем на vmix.

**Текст страницы НЕ вычитан:**
- `/phone-as-webcam/` — локали создали 2026-09-04 уже ПОСЛЕ прохода вычитки, и в `14413694`
  их файлов нет;
- раздел про плагин на `/stream-deck/` — добавлен позже прохода;
- замена «peer-to-peer» в 9 страницах × 34, ссылки на хабах.

⚠️ **Поправка к прежней записи.** Здесь стояло «/alternatives/vmix/ вычитан носителями
2026-09-05». Это преувеличение: 05.09 носители вычитали только ДОБАВЛЕНИЯ к странице —
карточку iOS/Android и переписанные под бюджет CJK-мета, — а не её текст. Проверяется одной
командой: `git show --name-only 14413694 | grep vmix` возвращает пусто.

**Вычитаны отдельные правки (не тексты страниц):** карточка iOS/Android на vmix ×34 и CJK-мета
(2026-09-05); 13 локальных заголовков и H1 по реальному спросу и 4 заголовка uk/tr (2026-09-06).

Правило на будущее: «страница вычитана» и «правка на странице вычитана» — разные утверждения.
В долге писать первое можно, только если файл страницы был в коммите прохода.

### 🔴 Нашёл одно унаследованное от шаблона поле — ищи ВСЕ (2026-09-05)

Семь страниц (`alternatives/manycam · restream · streamyard · vmix`, `stream-deck`,
`phone-as-webcam`, `multi-camera`) собраны из шаблона страницы `/alternatives/obs/` и
утащили её поля. Ловим их по одному уже третий раз:

1. **hreflang** (2026-09-03) — 3 страницы × 34 объявляли своими переводами локали чужой страницы.
2. **og:image** (коммит `e36109e8`) — «seven new pages carried the OBS page's og:image — **245 файлов**».
3. **Комментарий с SEO-таргетингом** (2026-09-05) — те же 245 файлов несли
   `<!-- SEO targeting: "obs alternative" (500 vol, KD 0) … Meld Studio currently captures this -->`.
   Он прямо противоречил `meta keywords` строкой ниже и врал следующему редактору о том, подо
   что заточена страница. **ЗАКРЫТО 2026-09-05:** каждая из шести страниц получила свой
   комментарий — ключи из её же keywords, цифры только измеренные, с источником и датой;
   где спрос не мерили, так и написано.
4. **Комментарий `<!-- Schema.org: … + ItemList comparison -->`** (2026-09-05) — найден при
   том самом сплошном диффе `<head>`. `ItemList` есть только на `/alternatives/obs/`; семь
   страниц заявляли его, не имея. Заодно нашлись шесть страниц вне шаблона с тем же дефектом,
   и там комментарий `SoftwareApplication (brand with rating)` подталкивал вернуть
   `aggregateRating`, вырезанный по правилу #7. Исправлены все 454 файла.
5. **48 строк мёртвого CSS** (2026-09-05) — 23 класса (`adv*`, `alt-*`, `bonus*`, `step*`,
   `reason-num`, `tag`, `vs`) не встречаются в разметке ни одного из 245 файлов. Среди них
   `.adv .vs::before{content:'OBS: '}` — бейдж с чужим брендом под комментарием
   «Advantages list», готовый вылезти на страницу про ManyCam, как только кто-то напишет
   `class="adv"`. Удалено; рендер проверен на 390/1440 px в EN, ar, he, ja.

Почему каждый раз проскакивает: поле валидно синтаксически, ссылка рабочая, картинка
существует, комментарий вообще невидим, мёртвое правило ни на что не влияет. Ни одна проверка
«страница не сломана» этого не видит — видно только сравнением с ИСХОДНОЙ страницей шаблона.

**Правило:** нашёл одно поле, унаследованное от шаблона, — не чини его в одиночку. Продиффь весь
`<head>` (и комментарии, и `<style>` в нём) новой страницы против страницы-шаблона и вычисти всё
разом. Волумы в таком комментарии не выдумывать: если для страницы спрос не мерили, так и писать.

**Теперь это ловит код — `page_audit.py` T25** (проверено внедрением дефекта, а не предположением):
комментарий `SEO targeting`, ни один ключ которого не встречается в `keywords` страницы (её локали
или EN), и комментарий `Schema.org: …`, чей список типов не равен реальным `@type` в JSON-LD, —
оба 🔴. Мёртвый CSS правилом не покрыт: он не про SEO, ищется разово скриптом «классы в `<style>`
минус классы в разметке и в JS».

### 🔴 Проверка, которая мерила страницу ошибки Chrome (2026-09-05)

Мобильная проверка рапортовала «OK, scrollWidth 390/390» по страницам, которые вообще не открылись.
Две причины сразу:
1. `file://' + process.cwd() + path` — путь репозитория содержит пробел («SplitCam сайт») и кириллицу,
   Chrome такой URL не открывает. Только `pathToFileURL()`.
2. Скрипт автоопределения языка на АНГЛИЙСКОЙ странице честно уводит не-английский браузер на `/ru/…`;
   под `file://` это несуществующий путь. Headless Chrome берёт язык системы — задавать `--lang=en-US`
   плюс `navigator.language`/`languages`, то есть то, чем ходит Googlebot.
Страница ошибки Chrome имеет ровно один `<h1>` и `scrollWidth == innerWidth`, поэтому проверка её
не отличала. **Правило: любая проверка рендера обязана сначала доказать, что страница загрузилась**
(в `seo/mobile_check.js` — один `<h1>` + непустой `<title>` + `<footer>`), и только потом мерить.
Третий случай одного класса ошибок: «инструмент сказал 0» ≠ «проблем нет».

Когда инструмент починили, нашлись настоящие 6 страниц с горизонтальной прокруткой на 390px
(`/virtual-camera/`, оба `virtual-audio-*`, `/multistreaming/`, `/alternatives/`, `/for/`): декоративная
подсветка героя шириной 500px при `right:-100px` в контейнере без обрезки растягивала документ до
490–545px. Лечится тем же правилом, что уже стояло на `/products/`: `overflow-x: clip; overflow-y: visible`.

### 🔴 zsh: `$PAGES` в аргументах команды — третий укус (2026-09-05)

`node seo/mobile_check.js $PAGES` передал ВСЕ 26 путей одной строкой, и Chrome получил один
несуществующий файл «. products features …/index.html». Тот же механизм, что дважды ломал циклы по
локалям. **В zsh любой список в переменной раскрывать только как `${=VAR}`** — или собирать команду
в python. И скрипт, принимающий список, должен переживать падение одного элемента, а не весь прогон.

### 🔴 FAQ: ответы переведены, вопросы перепутаны местами (2026-09-05)

На `virtual-audio-mac` и `virtual-audio-windows` в 31 локали нашлись **190 английских вопросов FAQ
при полностью переведённых ответах**. Хуже: часть уже переведённых вопросов стояла НЕ В СВОИХ
слотах — на `fr/virtual-audio-windows` вопрос «Faut-il télécharger un pilote audio virtuel ?» стоял
над ответом про аудиомикшер, а вопрос про микшер — над ответом «Oui, c'est gratuit». Тот же
французский вопрос при этом дублировал английский из первого слота.

Почему ни одна проверка не видела: `faq_sync` сверяет разметку FAQPage с ВИДИМЫМ текстом — а видимый
текст и был испорчен, так что разметка честно повторяла ошибку и рапортовала «0 расхождений».
Порядок блочных тегов не менялся, ссылки целы, JSON парсится. Это снова след жадной регулярки при
частичном переводе: вопросы записали в чужие слоты.

**Правило:** вопрос FAQ проверяется не сам по себе, а в паре со СВОИМ ответом и со СВОИМ слотом в EN.
Механически это ловится двумя признаками: (1) вопрос локали дословно равен английскому при
переведённом ответе; (2) на одной странице два одинаковых по смыслу вопроса. Оба признака стоит
добавить в `seo/page_audit.py` отдельным правилом; до тех пор — сверять вручную при любой правке FAQ.
